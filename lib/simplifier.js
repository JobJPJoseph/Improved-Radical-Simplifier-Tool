class Simplifier {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (obj.type === "Exponentiation") {
        ...
        if (obj.base.type === "NumberLiteral") {
          norm.coeff = String(Number(norm.coeff) * Number(this.simplifyExponent(obj).value));
        }
  ...
  rebuildAstFromTree(tree) {
    ...
    // Reduce a demonstrating a DFT on rebuilding the tree. It should be BFT
    // Let look in detail
    // I think we can just import the Parser class to fix this.
