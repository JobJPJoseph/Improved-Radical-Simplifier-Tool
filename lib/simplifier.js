class Simplifier {
  ...
  rebuildAstFromTree(tree) {
    ...
    // DO NOT FLATTEN IMPLICITS!!!

    // Now splits it by term
    for (let i = 0; i < flattenL.length; i++) {
      termsL.push(this.flattenBinary(flattenL[i]));
    }

    for (let j = 0; j < flattenR.length; j++) {
      termsR.push(this.flattenBinary(flattenR[j]));
    }
  ...
  flattenBinary(tree) {
    ...
    if (tree.type === "BinaryExpression") {
      return [...this.flattenBinary(tree.left), ...this.flattenBinary(tree.right)];
    }
  ...
  squareCoefficient(tree) {
    let num = Number(tree.value);
    let coeff = 1;
    ...
    let wholeNumber = Math.sqrt(coeff);
    let leftOver = num / coeff;

    return {coeff: wholeNumber, remainder: leftOver};
  }
